...
驱动实测。除 `LOAD-1 /healthz p95` 探针（预热 + best-of-3 取最优轮次后作为单元
测试进入 `make quality`）外，其余计时类探针**不进入 `make quality`**（环境相关，
不可作门禁），由人工/CI 按需执行并留档。
...
- 计时类结果不写审计链；除 `LOAD-1 /healthz p95`（预热 + best-of-3）外，
  其余计时类结果不参与质量门禁。
