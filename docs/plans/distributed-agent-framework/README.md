...
| 文件 | 版本 | 状态 | 摘要 |
| --- | --- | --- | --- |
| `design-proposal.md` | **v0.4** | **产品级 + 工程级双一致蓝图（推荐作为实施基线）** | 在 v0.3 基础上吸收第三轮审查（A 类 12 条主要 + B 类 7 条润色）；累计落地 38 条审查结论；自评 4.95/5 |
...
| Manifest | §5.1 草案 + §5.2 闭集 | **+ §5.3 manifest-checker 输入输出契约 + T1..T5** |
...
- `design-proposal-v0.3.md` 占位（v0.3 完整内容已合并到 v0.4 §19.1/§19.4）
...
| 主题 | 负责模块 | 里程碑 | 测试要求 | 估算 |
| --- | --- | --- | --- | --- |
| `manifest-checker.md` | `src/coevo/framework/manifest/` | **M1a (2 周)** | T1..T5 | 2 周 |
...
5. **启动 M1a 切片**：manifest-checker + 5 项测试

---

> **结论**：v0.4 是"产品级 + 工程级双一致"方案。修复了 v0.3 评审中发现的 12 处内部不一致（A1..A12 主要），同时加固 7 处局部润色（B1/B4/B6/B7/B8/B10/B11）。从 v0.1 评审到 v0.4，累计 38+ 条审查结论全部落地，自评 4.95/5，可作为 M1..M9 实施基线。
>
> **v0.4 不再频繁大改**：仅在 v0.5（v0.4 BACKLOG 全部完成或新外部约束触发）时聚合演进。
