...
### 2.2.1 令牌使用安全指引（PRODUCT-REVIEW T-17）

- **生命周期**：默认不活动超时 8 小时（`COEVO_SESSION_TIMEOUT_SEC`，演示模式可经
  `run_demo.py --session-hours` 放宽）；服务端仅存 SHA-256 摘要，stdout 一次显
  示后不落盘。
- **浏览器端存储**：令牌存于 `sessionStorage`（关闭标签即清）；页面加载后
  `history.replaceState` 移除地址栏 `?token=`，地址栏不残留令牌。
- **复制/分享注意**：驾驶舱"复制地址"只复制不带令牌的基地址（含当前视图 hash）；
  不要分享带 `?token=` 的完整地址——任何拿到该 URL 的人即获得会话。令牌失效后
  页面自动回到登录面板，粘贴新令牌即可重连，无需重启服务。
- **失窃处置**：本地环回 + 短超时降低暴露面；如怀疑令牌泄露，重启驾驶舱服务即
  清空全部会话。
