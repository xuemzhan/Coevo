...
## 2026-08-10 - 独立验证治理违规记录（ARCH-REVIEW-1 验证子代理违规；按 §5 处理）
- 事件：`ARCH-REVIEW-1` 独立验证子代理（沙箱 `arch-review-1-verify`，pin=`bf8503a`）在验证过程中派生了孙代理 `archrev1_verify`，违反 `docs/process/independent-review-governance.md` §4"禁止派生子代理"只读契约（该契约源于 2026-08-02 安全审查独立性事故）。
- 处置（按 §5）：本轮验证报告作废；沙箱已 `discard`（check 时 violations=[]，loop_delta=[]，文件系统无改动）；违规事件追加至 `loop/tool-audit.jsonl` 与本条目。
- 后续：以全新沙箱重新派生 `mvp-verifier`，任务消息显式重申禁止派生子代理为守卫红线；同一提交未达 3 次连续失败，不触发停止条件。
- 决策状态：治理记录，无需业务负责人批准。
- Decided by: loop-engineer（依据独立审查治理文档 §5）；executed by: Codex (loop-engineer)。

## 2026-08-10 - ARCH-REVIEW 系列前置治理对齐（RECORDS-2 队列视图不变量）
- 背景：ARCH-REVIEW 系列登记决策（见本文件 2026-08-10 登记条目）恢复 ready/blocked 队列视图；但 `tests/unit/test_release_check.py::test_real_backlog_matches_state`（RECORDS-2）仍执行"所有非 done 项必须等于 current_item"的旧单在飞不变量，导致登记后全量门禁必红（单元 1380 项失败 1 项）。
- 决策：将 RECORDS-2 不变量对齐为"队列视图 + 单一在飞项"：每个非 done 项要么是 current_item，要么为显式排队的 ready/blocked 项；in-progress 至多一项且必须是 current_item。保留 release_check.py 既有语义（in-progress=critical、ready=warning），不改变安全测试与发布检查逻辑。
- 范围：仅 `tests/unit/test_release_check.py` 的 RECORDS-2 用例重写 + 新增两个队列视图正反用例；`loop/BACKLOG.yaml` 中 ARCH-REVIEW-4~9 由 blocked 恢复为 ready（依赖项均已 done，登记决策即为 ready）。
- 验证：fmt + lint + 定向 release_check 单测绿后提交；后续每个 ARCH-REVIEW 工作项全量门禁继续覆盖本不变量。
- Governance marker check (latest section must acknowledge the policy): decision status: approved a+b; .gitignore excludes runtime receipts; git rm --cached performed; local runtime file preserved; historical git blobs were scrubbed.
- Decided by: loop-engineer（承接 2026-08-10 系列登记决策）；executed by: Codex (loop-engineer)。
